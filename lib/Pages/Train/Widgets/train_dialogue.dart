...
//    String noOfLuaggage;
  ...
  @override
  _TrainDialogState createState() => _TrainDialogState();
}

class _TrainDialogState extends State<TrainDialog> {
...
//  final TextEditingController driverNumberController = TextEditingController();
  ...
  Future<void> _saveMenu(BuildContext context) async {
    if (validate()) {
      final menuProvider = Provider.of<TrainProvider>(context, listen: false);
  ...
  bool validate() {
    ...
    // if (widget.isNew) {
    //   if (_image == null) {
    //     showSnackBar("Please add image of Train Station");
    //     return false;
    //   }
    // }
  ...
  Future<void> updateTrainRecord() async {
    if (validate()) {
      final menuProvider = Provider.of<TrainProvider>(context, listen: false);

      String result = await menuProvider.updateTrainRecord(
          //_image,
          widget.id,
          pickUpController.text.trim(),
          dropOfController.text.trim(),
    ...
    return AlertDialog(
      ...
      actions: <Widget>[
        ElevatedButton(
          ...
          onPressed: () =>
              widget.isNew ? _saveMenu(context) : updateTrainRecord(),
        ),
        ElevatedButton(
