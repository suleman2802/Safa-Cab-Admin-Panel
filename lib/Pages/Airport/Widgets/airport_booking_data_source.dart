...
import '../Widgets/airport_dialogue.dart';

// Create a Data Source class by extending DataTableSource
class AirportBookingDataSource extends DataTableSource {
  List<Airport> _bookings;
  int _selectedCount = 0;
  BuildContext context;
  var deleteAirportById;

  AirportBookingDataSource(this._bookings,this.context,this.deleteAirportById);

  @override
  DataRow? getRow(int index) {
    assert(index >= 0);
    if (index >= _bookings.length) return null;
    final Airport booking = _bookings[index];
  ...
  void sort<T>(Comparable<T> Function(Airport c) getField, bool ascending) {
    _bookings.sort((Airport a, Airport b) {
      if (!ascending) {
        final Airport c = a;
        a = b;
        b = c;
