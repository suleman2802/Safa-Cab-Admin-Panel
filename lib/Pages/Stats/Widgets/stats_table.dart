...
import 'package:flutter_dashboard/Pages/Stats/Widgets/stats_data_source.dart';
...
class _StatsTableState extends State<StatsTable> {
  ...
  StatsDataSource? _statsDataSource;
  DateTime? _pickFromDate;
  DateTime? _pickToDate;
  List<Stats>? list;
  List<Stats>? filteredList;
  ...
  void initState() {
    ...
    //   _statsDataSource!.search(_searchController.text);
    // });
  ...
  void _filterStats() {
    setState(() {
      if (_searchController.text.isEmpty) {
        ...
        filteredList = list!.where((stats) {
          return stats.noOfBookings
                  .toString()
                  .toLowerCase()
      ...
      _statsDataSource = StatsDataSource(filteredList!);
    });
  }

  fetchStatsRecord() async {
    list = [];
    list = await Provider.of<StatsProvider>(context, listen: true)
        .fetchReviewRecord();
    _statsDataSource = StatsDataSource(list!);
  }

  // deleteStatsRecordById(int id)async{
  //   String result = await Provider.of<StatsProvider>(context, listen: false)
  //       .deleteStatsById(id);
  //   if (result == "Ok") {
  ...
  void _sort<T>(Comparable<T> Function(Stats d) getField, int columnIndex,
      bool ascending) {
    _statsDataSource!.sort(getField, ascending);
    setState(() {
    ...
    return Column(
      ...
      children: [
        ...
        SingleChildScrollView(
          child: FutureBuilder(
              future: list == null ? fetchStatsRecord() : null,
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return PaginatedDataTable(
                    ...
                    horizontalMargin: !Responsive.isMobile(context) ? 80 : 20,
                    columnSpacing: !Responsive.isMobile(context) ? 110 : 15,
                    // actions: [
                    //   ElevatedButton(
                    //     onPressed: () {},
                    ...
                    //   ),
                    // ],
                    header: const Text('Revenue Record'),
                    ...
                    columns: <DataColumn>[
                      DataColumn(
                        ...
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Stats d) => d.year, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Month'),
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Stats d) => d.month, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('No Of Bookings'),
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Stats d) => d.noOfBookings,
                            columnIndex,
                            ascending),
                      ),
                      DataColumn(
                        label: const Text('No of Ziyarts'),
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Stats d) => d.noOfZiyarats,
                            columnIndex,
                            ascending),
                      ),
                      DataColumn(
                        label: const Text('No of Packages'),
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Stats d) => d.noOfPackages,
                            columnIndex,
                            ascending),
                      ),
                      DataColumn(
                        ...
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Stats d) => d.revenue, columnIndex, ascending),
                      ),
                      // const DataColumn(
                      //   label: Text('Delete'),
                      // ),
