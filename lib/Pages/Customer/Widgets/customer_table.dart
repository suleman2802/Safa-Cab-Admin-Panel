...
import "../Widgets/customer_data_source.dart";
...
class _CustomerTableState extends State<CustomerTable> {
  ...
  CustomerDataSource? _customersDataSource;
  List<Customer>? list;
  ...
  void _filterCustomer() {
    setState(() {
      if (_searchController.text.isEmpty) {
        filteredCustomer = list;
      } else {
        filteredCustomer = list!.where((customer) {
      ...
      _customersDataSource =
          CustomerDataSource(filteredCustomer!, context, deleteCustomerRecord);
    });
  }

  deleteAllCustomerRecord() async {
    String result = await Provider.of<CustomerProvider>(context, listen: false)
  ...
  deleteCustomerRecord(int id) async {
    String result = await Provider.of<CustomerProvider>(context, listen: false)
  ...
  Future<void> getCustomerRecord() async {
    list = [];
    list = await Provider.of<CustomerProvider>(context).fetchCustomerRecord();
    _customersDataSource =
        CustomerDataSource(list!, context, deleteCustomerRecord);
  }
  ...
  void initState() {
    ...
    //   _customersDataSource!.search(_searchController.text);
    // });
    _searchController.addListener(_filterCustomer);
  }
  ...
  void _sort<T>(Comparable<T> Function(Customer d) getField, int columnIndex,
      bool ascending) {
    _customersDataSource!.sort(getField, ascending);
    setState(() {
    ...
    return Column(
      ...
      children: [
        ...
        SingleChildScrollView(
          child: FutureBuilder(
              future: list == null ? getCustomerRecord() : null,
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return PaginatedDataTable(
                    ...
                    horizontalMargin: !Responsive.isMobile(context) ? 70 : 20,
                    columnSpacing: !Responsive.isMobile(context) ? 100 : 20,
                    actions: [
                      ElevatedButton(
                        onPressed: deleteAllCustomerRecord,
                    ...
                    columns: <DataColumn>[
                      ...
                      //         (Customer d) => d.name, columnIndex, ascending),
                      //),
                      DataColumn(
                      ...
                      DataColumn(
                        ...
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Customer d) => d.email, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Customer Type'),
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Customer d) => d.type, columnIndex, ascending),
                      ),
                      // const DataColumn(
                      //   label: Text('Edit'),
                      // ),
