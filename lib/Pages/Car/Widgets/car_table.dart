...
import "../Widgets/car_data_source.dart";
...
class _CarTableState extends State<CarTable> {
  ...
  CarDataSource? _carsDataSource;
  List<Car>? carRecord;
  List<Car>? filteredCarRecord;

  // Future<void> getCarRecord() async {
  //   carRecord = [];
  //   carRecord = await Provider.of<CarProvider>(context).fetchCarRecord();
  //   print(carRecord);
  //   _carsDataSource = CarDataSource(carRecord!, context, deleteCarRecord);
  // }

  Future<void> getCarRecord() async {
    carRecord =
        await Provider.of<CarProvider>(context, listen: true).fetchCarRecord();
    filteredCarRecord = carRecord;
    _carsDataSource =
        CarDataSource(filteredCarRecord!, context, deleteCarRecord);
    //setState(() {});
  }

  deleteCarRecord(int id) async {
    String result = await Provider.of<CarProvider>(context, listen: false)
        .deleteCarById(id);
  ...
  void _filterCars() {
    setState(() {
      if (_searchController.text.isEmpty) {
        ...
        filteredCarRecord = carRecord!.where((car) {
          return car.carName
                  .toLowerCase()
  ...
  void _sort<T>(
      Comparable<T> Function(Car d) getField, int columnIndex, bool ascending) {
    _carsDataSource!.sort(getField, ascending);
    setState(() {
    ...
    return Column(
      ...
      children: [
        ...
        SingleChildScrollView(
          child: FutureBuilder(
              future: carRecord == null ? getCarRecord() : null,
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return PaginatedDataTable(
                    ...
                    horizontalMargin: !Responsive.isMobile(context) ? 50 : 12,
                    columnSpacing: !Responsive.isMobile(context) ? 60 : 20,
                    actions: [
                      ElevatedButton(
                        onPressed: () {
                          showDialog(
                    ...
                    columns: <DataColumn>[
                      ...
                      DataColumn(
                        ...
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Car d) => d.carName, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Car Number'),
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Car d) => d.carNumber, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Driver Name'),
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Car d) => d.driverName, columnIndex, ascending),
                      ),
                      // DataColumn(
                      //   label: const Text('Driver Number'),
                      //   onSort: (columnIndex, ascending) => _sort<String>(
                      //       (Car d) => d.driverNumber, columnIndex, ascending),
                      // ),
                      DataColumn(
                        label: const Text('Car Model'),
                        numeric: true,
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Car d) => d.carModel, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Quantity'),
                        numeric: true,
                        onSort: (columnIndex, ascending) => _sort<num>(
                                (Car d) => d.totalQyt, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Seats'),
                        numeric: true,
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Car d) => d.noOfSeats, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Luggage'),
                        numeric: true,
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Car d) => d.noOfLuggage, columnIndex, ascending),
                      ),
                      const DataColumn(
                        label: Text('Edit'),
                      ),
