...
import 'package:flutter_dashboard/Pages/Booking/Provider/booking_provider.dart';
import 'package:provider/provider.dart';
import '../../../General Widgets/Responsive.dart';
import '../Model/Booking.dart';
import "../Widgets/booking_data_source.dart";
...
class _BookingTableState extends State<BookingTable> {
  ...
  BookingDataSource? _bookingsDataSource;
  ...
  void initState() {
    ...
    _searchController.addListener(() {
      _bookingsDataSource!.search(_searchController.text);
    });
  }

  Future<void> getBookingRecord() async {
    list = [];
    list = await Provider.of<BookingProvider>(context, listen: true)
        .fetchBookingRecord();
    // print("Customer Name : ${list![0].customerName} booking id : ${list![0].id} booking type  : ${list![0].bookingType} price : ${list![0].price}");
    _bookingsDataSource =
        BookingDataSource(list!, context, deleteBookingRecord);
  }

  deleteBookingRecord(int id) async {
    String result = await Provider.of<BookingProvider>(context, listen: false)
  ...
  void _sort<T>(Comparable<T> Function(Booking d) getField, int columnIndex,
      bool ascending) {
    _bookingsDataSource!.sort(getField, ascending);
    setState(() {
    ...
    return Column(
      ...
      children: [
        ...
        SingleChildScrollView(
          child: FutureBuilder(
              future: list == null ? getBookingRecord() : null,
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return PaginatedDataTable(
                    ...
                    //horizontalMargin: !Responsive.isMobile(context) ? 50 : 20,
                    ...
                    //   ),
                    // ],
                    header: const Text('Booking Record'),
                    ...
                    columns: <DataColumn>[
                      DataColumn(
                        ...
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Booking d) => d.bookingId, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Booking type'),
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Booking d) => d.bookingType,
                            columnIndex,
                            ascending),
                      ),
                      DataColumn(
                      ...
                      DataColumn(
                        ...
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Booking d) => d.dateTime, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text('Status'),
                        onSort: (columnIndex, ascending) => _sort<String>(
                            (Booking d) => d.status, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text("Discounted"),
                        numeric: true,
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Booking d) => d.discount, columnIndex, ascending),
                      ),
                      DataColumn(
                        label: const Text("Price"),
                        numeric: true,
                        onSort: (columnIndex, ascending) => _sort<num>(
                            (Booking d) => d.price, columnIndex, ascending),
                      ),
                      const DataColumn(
                        label: Text('Edit'),
                      ),
